...
bbb-conf --restart
sed -i "s/bbb.s-neshaa.ir/$new_domain/g" /etc/zabbix/zabbix_agentd.conf
service zabbix-agent restart
docker exec greenlight-v2 bundle exec rake user:create["$my_username","$my_email","$my_password","admin"]
bbb-conf --secret
